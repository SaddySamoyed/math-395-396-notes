...
\begin{proposition}{Higher prospective of Directional Derivative}
...
\begin{theorem}
    ...
    其中每一列就是 $D_{e_i} f = \frac{\partial}{\partial x_i} f$. 因而对于每个输入 $ c \in \mathbb{R}^n$，我们得到的输出都是 $\sum_{i=1}^n c_i \frac{\partial}{\partial x_i} f(c) \in \mathbb{R}^m$.\\
...
\begin{definition}{higher-order partial derivatives}
    ...
    recursively define: 
    $$
    {\partial^d \over \partial x_{k_1}\partial x_{k_2}\cdots \partial x_{k_d}}
    $$
...
\section{$C^r$ class}
\begin{definition}
    如果 $f$ 的所有 $r^\text{th}$-order partial derivatives 都 \textbf{exist 并且 ctn}，则称 $f$ 是 $C^r$ 的.
    \\$f \in C^r(\text{dom}(f))$
\end{definition}
...
\begin{remark}
    对于 f 在点 $x$ 附近 being $C^1$ 的解释: $f$ 在点 $x$ 附近，每个 $e_i$ 切向量上的变化率函数都是连续的. 所以我们有一种直觉：$f$ 在这个点上是可以被一个 linear map 近似的.
\end{remark}
\end{document}